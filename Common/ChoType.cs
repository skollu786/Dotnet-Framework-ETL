    ...
    public static class ChoType
    {
        ...
        public static bool IsDynamicType(this Type type)
        {
            ...
            if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Dictionary<,>))
                return type.GetGenericArguments()[1] == typeof(object);
            ...
            //    || typeof(IDictionary).IsAssignableFrom(type) || (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(IDictionary<,>) && type.GetGenericArguments()[1] == typeof(object));
        }
        ...
        public static bool CanCastToPrimitiveType(this Type from, out Type to)
        {
            to = null;

            if (CanCast(from, typeof(string)))
            {
                to = typeof(string);
                return true;
            }
            else if (CanCast(from, typeof(decimal)))
            {
                to = typeof(decimal);
                return true;
            }
            else if (CanCast(from, typeof(double)))
            {
                to = typeof(double);
                return true;
            }
            else if (CanCast(from, typeof(float)))
            {
                to = typeof(float);
                return true;
            }
            else if (CanCast(from, typeof(ulong)))
            {
                to = typeof(ulong);
                return true;
            }
            else if (CanCast(from, typeof(long)))
            {
                to = typeof(long);
                return true;
            }
            else if (CanCast(from, typeof(uint)))
            {
                to = typeof(uint);
                return true;
            }
            else if (CanCast(from, typeof(int)))
            {
                to = typeof(int);
                return true;
            }
            else if (CanCast(from, typeof(ushort)))
            {
                to = typeof(ushort);
                return true;
            }
            else if (CanCast(from, typeof(char)))
            {
                to = typeof(char);
                return true;
            }
            else if (CanCast(from, typeof(byte)))
            {
                to = typeof(byte);
                return true;
            }
            else if (CanCast(from, typeof(sbyte)))
            {
                to = typeof(sbyte);
                return true;
            }
